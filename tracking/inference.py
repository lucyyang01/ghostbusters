...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminatedVariable in eliminationOrder:
            factorList, joinedFactor = joinFactorsByVariable(factorList, eliminatedVariable)
            if len(joinedFactor.unconditionedVariables()) > 1:
                factorList.append(eliminate(joinedFactor,eliminatedVariable))
        joinedFactor = joinFactors(factorList)
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        #if ghost is in jail observation is None
        #busters.getObservationProbability(noisyDistance, trueDistance) -> prob(noisy is true)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        legalPos = float(len(self.legalPositions))
        distance = legalPos/float(self.numParticles)

        for coeff in range(self.numParticles):
            self.particles.append(self.legalPositions[int(coeff*distance)])
        "*** END YOUR CODE HERE ***"
